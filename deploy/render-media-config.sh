...
Environment (optional):
  LIVEKIT_NODE_IP / XINCHAT_PUBLIC_HOST   browser-reachable host
  LIVEKIT_API_KEY / LIVEKIT_API_SECRET  must match the API process
  TURN_USER / TURN_PASS                 coturn credentials
  XINCHAT_ENV=production                  same as --strict (refuse default secrets)
EOF
  exit "${1:-0}"
}
...
if [[ "$XINCHAT_ENV_VAL" == "production" ]]; then
  STRICT=1
fi
if [[ "$STRICT" -eq 0 && -f "$ROOT/deploy/xinchat-api.env" ]]; then
  env_line="$(grep -E '^XINCHAT_ENV=' "$ROOT/deploy/xinchat-api.env" | tail -n1 || true)"
  case "$env_line" in
    XINCHAT_ENV=production|XINCHAT_ENV=PRODUCTION) STRICT=1 ;;
  esac
fi
...
TURN_USER="${TURN_USER:-xinchat}"
TURN_PASS="${TURN_PASS:-xinchatturnsecret}"
...
replace_all "$OUT/livekit.yaml" "__XINCHAT_HOST__" "$HOST"
...
replace_all "$OUT/coturn.conf" "__XINCHAT_HOST__" "$HOST"
...
if [[ -f "$ROOT/deploy/certs/xinchat.crt" && -f "$ROOT/deploy/certs/xinchat.key" ]]; then
  SCHEME="wss"
  LIVEKIT_PORT="7443"
fi
# Callers behind HTTPS should set LIVEKIT_URL=wss://… themselves in xinchat-api.env.
...
# Keep systemd API env in sync with LiveKit URL/keys (merge into xinchat-api.env).
API_ENV="$ROOT/deploy/xinchat-api.env"
if [[ -f "$API_ENV" ]]; then
  tmp="$(mktemp)"
...
Next:
  ...
  # API: EnvironmentFile deploy/xinchat-api.env (LiveKit keys merged when present)
