...
async function ensureRemoteUiHydrated(win, webOrigin) {
  ...
  const probe = `
    (async function () {
      ...
      if (/Starting XinChat|Starting Rchat|Starting Qchat/i.test(text) && !document.querySelector("input, .auth-wrap")) {
        return {
          ok: false,
          reason: "stuck-splash",
    ...
    const watchdog = `
      (function () {
        try {
          ...
          if (text !== "Loading…" && text !== "Loading..." && text.indexOf("Starting XinChat") === -1 && text.indexOf("Starting Rchat") === -1) return;
          setTimeout(function () {
