...
func displayPlatform(platform, deviceName, deviceType, ua string) string {
	...
	if !weakPlatformLabel(deviceName) && !strings.EqualFold(deviceName, "web") {
		// e.g. "XinChat Desktop (Windows 11)" → Windows 11
		for _, prefix := range []string{"XinChat Desktop (", "Rchat Desktop (", "Qchat Desktop ("} {
		...
		if strings.HasPrefix(deviceName, "XinChat Mobile (") || strings.HasPrefix(deviceName, "Rchat Mobile (") || strings.HasPrefix(deviceName, "Qchat Mobile (") {
			return deviceName
		}
	}
